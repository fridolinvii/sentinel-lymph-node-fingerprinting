...
mnew = 0;
delta = [];
for time =  32 %[1 2 4 8 16 32]
    
    
    
    
    for pattern = 6% [1 2 3 4 5 6 7 8 9] %2 3
        ...
        load(['image_bearbeitet/pattern_',num2str(pattern),'/uncut/noWalls_N_pattern_',num2str(pattern),'_expt_',num2str(time),'.mat'])
...
% %             ylabel('C(d)','fontsize',fs)
        ...
        delta = [delta;time,pattern,mnew,mold];
        
        pause(1)
    end
end

% save('SOURCE_LL','sources','-v7.3')
