...
for rot = [0 25]
    for dist = 0:5:25
        ...
        for x_ = 12:16%1:max(x(:))
            Str = ['radiantSourceAo/special_experiment/rot_',num2str(rot),'/dist_',num2str(dist),'/radiantSource_expt_',num2str(t)];
            load(Str)
            m = max(radiantSource(:));
        ...
        title(['rot\_',num2str(rot),'\_dist\_',num2str(dist),'\_x\_12:16'])
        xlabel('z')
        ylabel('y')
        view([0,0,11])
